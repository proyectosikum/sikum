...
class MaternalDataFormNotifier extends ChangeNotifier {
  ...
  void updateGravidity(String value) {
    gravidity = value;
    errors['gravidity'] = validateNumeric(
      value,
      fieldName: 'Cantidad de gestas',
    );
    notifyListeners();
  }
  ...
  void updateCesareans(String value) {
    cesareans = value;
    errors['cesareans'] = validateNumeric(
      value,
      fieldName: 'Cantidad de cesáreas',
    );
    notifyListeners();
  }
  ...
  void updateAbortions(String value) {
    abortions = value;
    errors['abortions'] = validateNumeric(
      value,
      fieldName: 'Cantidad de abortos',
    );
    notifyListeners();
  }
  ...
  void updateBloodType(String value) {
    bloodType = value;
    errors['bloodType'] = validateNotEmpty(
      value,
      fieldName: 'Grupo y factor de la madre',
    );
    notifyListeners();
  }
  ...
  bool validateFirstHalfTests() {
    ...
    for (final testName in firstHalfTests) {
      if (testResults[testName] == null || testResults[testName]!.isEmpty) {
        testErrors['${testName}_result'] =
            'Seleccione un resultado para $testName';
        isValid = false;
      }
      ...
      if (_requiresDate(result)) {
        if (testDates[testName] == null || testDates[testName]!.isEmpty) {
          testErrors['${testName}_date'] =
              'Seleccione una fecha para $testName';
          isValid = false;
        }
      } else {
  ...
  bool _requiresDate(String? result) {
    ...
    return result.toLowerCase() == 'positiva' ||
        result.toLowerCase() == 'negativa';
  }
...
final maternalDataFormProvider =
    ChangeNotifierProvider.family<MaternalDataFormNotifier, String>((
      ref,
      patientId,
    ) {
